...
Aunque visualmente se aprecia una diferencia clara entre los tres niveles de dificultad, especialmente entre la condición Alta respecto de Baja y Media, es necesario aplicar un análisis post-hoc para determinar si estas diferencias son estadísticamente significativas, ya que el gráfico por sí solo no permite concluirlo con total certeza.

```{r}
#Como se mencionó más arriba, debido a las medidas que se escapan del límite teórico,
#se utilizará un nivel de significancia más estricto de α = 0.025
...
El procedimiento post-hoc HSD de Tukey, aplicado con un nivel de confianza del 97.5% (α = 0.025), reveló diferencias significativas entre los tres niveles de dificultad analizados. En particular, se observó que:
